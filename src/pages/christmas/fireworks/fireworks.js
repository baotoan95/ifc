...
export class Fireworks {
  ...
  initialize = () => {
    ...
    setInterval(this.createFirework, this.timer);
  ...
  clearContext = () => {
    if(!this.library.background) {
      this.mainContext.fillStyle = "rgba(0, 0, 0, 1)";
      this.mainContext.fillRect(0, 0, this.viewportWidth, this.viewportHeight);
    }
    this.mainContext.drawImage(this.library.background, 0, 0, this.viewportWidth, this.viewportHeight);
  }
    ...
    while (a--) {
      ...
      if (firework.update()) {
        ...
        if (!firework.usePhysics) {
          ...
          if (Math.random() < 0.8) {
            FireworkExplosions.star(firework, this);
          } else {
            FireworkExplosions.circle(firework, this);
          }
        }
      }
 ...
 */
class Particle {
  // properties for animation and colouring
  GRAVITY = 0.06;
  alpha = 1;
  easing = Math.random() * 0.02;
...
var FireworkExplosions = {
   ...
   */
  circle: function (firework, owner) {
